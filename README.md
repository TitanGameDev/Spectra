...
**What it deliberately leaves to you** (can't be automated generically — see [Azure AD (Entra ID) setup](#azure-ad-entra-id-setup) below): creating the Azure AD app registrations, granting per-customer admin consent, and the Exchange Online PowerShell certificate for the Email Security tab. It no longer asks about Azure AD at all — that all happens afterward, through the app itself (see below). It also only *provisions* the MySQL database — the actual cutover from the default local SQLite database still happens through **Settings → Database** in the browser once you're signed in (see [Database](#database-sql-server--mysql) below), using the credentials the script printed.

**Non-interactive use**: every prompt (domain, TLS, firewall) is also an environment variable — set `SPECTRA_DOMAIN=app.example.com SPECTRA_SETUP_TLS=yes ...` before piping it through `bash` to skip prompts entirely; see the script's own header comment for the full list. Safe to re-run — it skips packages/users/certs that already exist and just re-pulls, rebuilds, and restarts the app, same as clicking **Update now** in Settings → Updates (see [One-click update](#one-click-update-settings--updates) below) once it's installed.

**Azure AD setup — through a setup wizard, not the install script**: visiting the app for the first time shows a setup wizard instead of the sign-in button, because there's no Azure AD app registration configured yet. [`deploy/setup-azure-ad.sh`](deploy/setup-azure-ad.sh) creates both App Registrations described in [Azure AD (Entra ID) setup](#azure-ad-entra-id-setup) below — permissions, redirect URIs, the API scope, and a fresh client secret — via `az login` (device-code flow, no local browser needed, so it works fine over SSH) and the Microsoft Graph API. Run it from anywhere with the Azure CLI (it'll install the CLI itself if missing) — it doesn't need to run on the server, and needs an account with Application Administrator (or Global Administrator) rights on the target tenant:

```bash
curl -fsSL https://raw.githubusercontent.com/TitanGameDev/Spectra/main/deploy/setup-azure-ad.sh | bash
```

It's safe to re-run against the same tenant — it reuses the existing app registrations by display name instead of creating duplicates, and only mints a new client secret when the backend app registration is genuinely new (secret values can't be read back after creation, so re-runs won't silently pile up unused secrets — if you've lost the original, it prints the `az ad app credential reset` command to get a new one). At the end, it'll ask for your Spectra instance's URL and either **POST the result there directly** (using the one-time setup token `install.sh` generated at `/etc/spectra/setup-token.txt`) — skipping any copy-pasting — or, if you leave that blank, print the values to paste into the setup wizard yourself. Either way, this replaces ever needing to SSH in and edit `backend.env` by hand; the values live in the database from here on, and can be changed again later from **Settings → Authentication** (which restarts the backend to apply the change).

Front-end admin consent for the frontend app's `User.Read.All`/`Group.Read.All` delegated permissions (see [Admin access](#admin-access) below) is attempted automatically at the end; it'll warn instead of failing if the signed-in account doesn't have rights to grant it, with the manual steps to do it yourself in the Azure Portal.
...
[`deploy/setup-azure-ad.sh`](deploy/setup-azure-ad.sh) automates everything in this section — see [Azure AD setup — through a setup wizard, not the install script](#deploy) above. Whichever way you get there, none of these values are ever typed into an env file by hand anymore: the app itself (setup wizard on first run, **Settings → Authentication** afterward) stores them in the database and uses them from there. What follows is the manual Azure Portal reference — still the fallback for anything the script can't do for you, like clicking **Grant admin consent** yourself if it warns instead of doing it automatically.
...
1. **Frontend (SPA)**
   ...
   - Note the **Application (client) ID** and **Directory (tenant) ID** → these go in the setup wizard (or Settings → Authentication).
...
2. **Backend (API)**
   ...
   - Note the **Application (client) ID** and create a **client secret** → both go in the setup wizard (or Settings → Authentication) too.
...
Neither `.env.example` has Tenant ID / Client ID / scope placeholders anymore — those aren't build-time or process-start config at all, they live in the database instead (see [Azure AD (Entra ID) setup](#azure-ad-entra-id-setup) above). What's left in these `.env` files (CORS origin, the EXO certificate, the update-automation paths) is genuinely fine to leave as local files for dev; for anything beyond that, move them into your secure vault (Key Vault, etc.) instead.
...
Open `http://localhost:5173`. The first time, with no Azure AD app registration configured yet, you'll see the setup wizard instead of a sign-in button — run [`deploy/setup-azure-ad.sh`](deploy/setup-azure-ad.sh) (works fine against a local dev backend too: point it at `http://localhost:5080` when it asks for your instance URL, and leave the setup token blank — `Setup:Token` is optional in Development) and either let it POST straight in or paste the values it prints. After that, click **Sign in with Microsoft** — this opens a popup (no automatic/silent SSO), you authenticate against your tenant, and the SPA gets an access token it attaches as a `Bearer` header to calls against the backend's `/api/me`, which validates the token per-request.
...
2. **Create a client secret** — same app → **Certificates & secrets** → **New client secret** → give it a description and expiry → **Add**. Copy the **Value** immediately (not the Secret ID) — it's only shown once. Enter it in the setup wizard (or **Settings → Authentication**), not an env file — it's encrypted and stored in the database from there.
3. **Add Application permissions** — same app → **API permissions** → **Add a permission** → **Microsoft Graph** → **Application permissions** (not Delegated) → add:
   ...
   - `ReportSettings.ReadWrite.All` — lets Spectra automatically disable a customer's "concealed report identities" tenant setting when it detects that's blocking mailbox usage data, instead of asking their admin to click through the Microsoft 365 admin center by hand — see the Mailboxes note below.
...
- The backend app registration's client secret (set via the setup wizard / Settings → Authentication) is only needed for this feature — the core sign-in flow never uses it. Leaving it unset is fine until you add your first customer; the error message you'll get is explicit about what's missing.

**Users tab sub-tabs**: Directory (job title, department, office, enabled/disabled, created date, and email aliases — every secondary address from Graph's `proxyAddresses` property, excluding the primary; uses `User.Read.All`, already granted, no new consent needed), Mailboxes (size, item count, archive enabled — needs `Reports.Read.All`, see above), and Licenses (assigned SKUs with friendly names via a small built-in lookup table, unlicensed users flagged distinctly). License details use `User.Read.All` — no extra permission there. If mailbox data is missing for every user on a customer whose consent otherwise succeeded, that's usually the `Reports.Read.All` permission not being granted yet, and the Mailboxes sub-tab says so directly — but **there's a second, easy-to-miss cause of the exact same symptom**: Microsoft conceals real user identities in usage-report data by default, tenant-wide, regardless of what permissions are granted. If `Reports.Read.All` is granted and the Graph call is succeeding (visible in the backend logs as a 200 from `reports/getMailboxUsageDetail`) but every mailbox still shows no data, that's almost certainly this — Spectra detects it automatically (a real row came back but matched no real user by UPN) and, with `ReportSettings.ReadWrite.All` granted, fixes it automatically too: the same collection run calls Graph's `PATCH /admin/reportSettings` to flip `displayConcealedNames` off, the same change the manual admin-center toggle makes, no admin click-through needed. It still takes Microsoft a few minutes to propagate either way, so the *next* collection is the one that picks up real data — the warning on this run just confirms the fix was applied. If `ReportSettings.ReadWrite.All` isn't granted (or a customer granted consent before this permission was added — existing consents don't retroactively pick up new permissions, they need **Grant consent** again from Settings → Customers), the warning falls back to the manual fix: Microsoft 365 admin center → **Settings → Org Settings → Services → Reports** → check **"Display Concealed user, group, and site names in all reports"** → Save. License/mailbox fetches are per-user Graph calls done with light parallelism during collection, so re-collecting a large tenant takes noticeably longer than just listing users did before this.
...
**Per customer, for VMs and App Services**: since there's no admin-consent screen to piggyback on, Settings → Customers has a **Copy Azure access command** button (`GET /api/customers/{id}/azure-role-command`) that copies a ready-to-run `az role assignment create` command, pre-filled with that customer's tenant ID and Spectra's app ID, scoped at the tenant's **root management group** rather than one subscription at a time — every Azure AD tenant has one by default (its ID is just the tenant ID itself), and RBAC assignments inherit down through everything under it. The customer's Azure admin pastes that one command into Cloud Shell (or their own terminal, signed into that tenant) with Owner/User Access Administrator rights — one run covers every subscription that exists today *and* any created later, no per-subscription follow-up needed. Prefer the manual portal route instead (**Subscription** or a specific **Management Group** → **Access control (IAM)** → **Add** → **Add role assignment** → **Reader** → search for "Spectra" → **Review + assign**) if a customer's tenant doesn't have management groups enabled, or you'd rather scope access to fewer subscriptions than the whole tenant. Until either is done, the Azure tab's Overview/Virtual Machines/App Services sub-tabs show an empty-subscriptions message rather than an error, since an empty result from `/subscriptions` is indistinguishable from "no subscriptions exist" and "Reader not assigned yet" at the API level.

**Reservations need a second, separate, higher-privilege role**: Reserved Instances/savings plans are a **tenant-level** resource, not scoped to any one subscription, so they use their own RBAC surface — **Reservations Reader**, assigned via a completely different screen: Azure Portal → **Home** → **Reservations** → **Role assignment** (top nav) → pick **Reservations Reader** → add Spectra's service principal. This needs the person doing it to already have delegate rights over reservations (typically whoever purchased them, or someone with elevated/User Access Administrator rights) — a bigger, rarer ask than the subscription Reader grant above, confirmed against [Microsoft's own reservation-permissions docs](https://learn.microsoft.com/en-us/azure/cost-management-billing/reservations/view-reservations). It's optional and degrades independently: everything else in the Azure tab works fine without it, and the Reservations sub-tab just explains what's missing until it's granted. Collection skips this entirely for a tenant with no Azure virtual machines — most customers are Microsoft 365-cloud-only with no Azure IaaS at all, so there's nothing to prompt for a role grant that wouldn't apply to them; the Reservations sub-tab says so directly rather than showing the "needs a role" message. A tenant that adds its first VM later picks this collection back up automatically on the next sync.
...
**Same design, reused for Azure AD changes**: saving Azure AD configuration (setup wizard or **Settings → Authentication**) writes a separate `$SPECTRA_DATA_DIR/restart-requested` flag rather than `update-requested` — a lighter `spectra-restarter.path`/`.service` pair (also installed by `install.sh`) just restarts `spectra.service`, skipping the git-pull/rebuild cycle entirely. A restart is needed there because the ASP.NET Core JWT bearer middleware reads the tenant/client ID once at process start; unlike the Database panel, there's no live no-restart path for that without depending on undocumented internals of the Microsoft.Identity.Web library, so this restarts every time instead — expect to be signed out briefly right after saving.
...
4. **Real secrets**: the Azure AD backend client secret isn't an env var at all anymore — it's encrypted (ASP.NET Core Data Protection) and stored in the database, which means the `keys/` directory (gitignored, holds the Data Protection key ring) needs to be backed up and persisted just like the database itself, or a restore leaves that secret permanently undecryptable. Move `Exo__CertificatePassword` and `Setup__Token` out of `.env` and into a proper secret store (Key Vault, systemd `EnvironmentFile` with restricted permissions, etc.) for anything beyond local dev.
